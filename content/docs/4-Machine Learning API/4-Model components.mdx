...
As we can see, the **MySKModel** class inherits from the **Model** class, which is a built-in class of <FlamaName />.
...
**MySKModel**, we would need to pass the model object as an argument to the constructor, as follows:

```python
with open("sklearn_model.flm", "rb") as f:
    model = pickle.load(f)
model = MySKModel(flama.load(f).model)
```

### ModelComponent

Now that we have the **Model** class implemented, we need the **ModelComponent** class. This class will be in charge
of instantiating the **MySKModel** class, and returning it via the **resolve** method. In this particular example,
...
Therefore, the **resolve** method will not be the one making the call to the constructor of the **MySKModel** class.
...
In short, to instantiate an object of the **MySKModelComponent** class, we need to pass the path to the model object as an
argument to the constructor. This path will be stored in the **self.\_model_path** attribute. The **self.model**
attribute is an instance of the **MySKModel** class, which is initialised with a **None** value, which makes it
...
How will we have to initialise the **MySKModelComponent** class? We will have to do it as follows:
...
**MySKModelComponent** class. The **metadata** method is also different, as it is now returning the **loaded**
...
**component** object by calling the **MySKModelComponent** class directly in the **component** class attribute.
...
<Image
  ...
  alt="docs-add-model-component-inspect-true"
  width="1600"
  height="1043"
/>
...
<Image
  ...
  alt="docs-add-model-component-inspect-false"
  width="1600"
  height="1043"
/>
