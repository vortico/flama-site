...
We suggest to save the previous example in a local file, say **examples/add_models.py**, and run it with the CLI in
development mode:
...
As we probably are guessing, we can add TensorFlow modesl by adding the following lines to the base application:
