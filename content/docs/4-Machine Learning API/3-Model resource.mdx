...
to be registered schemas in the **app**.

What do we need to make this example a working example? Let's check what the correct example
look like:
...
This example is fully functional, so you should be able to **flama-run** it after
adding the previous code snippet to the code introduced in the preamble.
