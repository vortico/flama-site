...
[here](https://github.com/perdy/flama/blob/develop/examples/add_model_resource.py).
...
class MyModel(ModelResource, metaclass=ModelResourceType):
    ...
    def metadata(self):
        ...
        return {"metadata": {
            "built-in": {"verbose_name": self._meta.verbose_name},
...
In the latter case, we can use the `--dev` flag to enable the development mode, which will allow us to use the interactive
debugger in case of an error. The following code shows how to run the app using the **flama run** command line:

```commandline
> flama run add_model_resource:app --dev
```
...
<Image src="/images/docs/doc-add-resources-final.png" alt="docs-addresource" width="1600" height="984" />
