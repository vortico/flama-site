...
This means, when we serve/add models, <FlamaName/> takes care of

--

Adding models to a <FlamaName /> App in the way introduced in the
[previous section](/docs/machine-learning-api/add-models) involves the creation of a `Resource` object (specifically, a
`ModelResource`) automatically for you, which represents an API resource, in the REST sense of the word. This
`ModelResource` defines the default way of interaction with the ML model itself by providing **predict** and **inspect**
(P&I) methods.
