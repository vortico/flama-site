...
concepts. In particular, we are going to start implementing the endpoints **/predict/** and **/inspect/**, which come by
default when using the tools seen so far to serve/add models automatically. With these two examples we will better
understand the _magic_ happening under the hood when using either the CLI **serve/start**, or the method
**app.models.add_model**. Then, we will show an example where ..
