...
2.  A **\_\_call\_\_** method:
    ...
    - This is the core of the middleware. It’s an asynchronous method that takes three arguments: **scope** (a dictionary containing information about the connection), **receive** (an awaitable callable to receive event messages), and **send** (an awaitable callable to send event messages).
    ...
    async def __call__(self, scope: types.Scope, receive: types.Receive, send: types.Send):
       # Code to run before the request is handled by the next app/middleware
        print(f"Middleware received request for path: {scope.get('path')}")
        print(f"Custom argument was: {self.custom_argument}")
...
if __name__ == "__main__":
    # Ensure flama.run is available; it's part of the main flama package.
    flama.run(flama_app=app, server_host="0.0.0.0", server_port=8000)
```

We can now run it and run a make **curl** to both endpoints, and check how the response comes with the expected header:
...
When you run this application and access the `/fast` or `/slow` endpoints, the response will include the **X-Process-Time-MS** header.
