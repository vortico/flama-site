...
In <FlamaName />, a **Route** essentially binds a URL path pattern and one or more HTTP methods, like <Label color="green">GET</Label>, <Label color="blue">POST</Label>, <Label color="orange">GET</Label>, or <Label color="red">DELETE</Label>; to a specific Python asynchronous function, often called an _endpoint_ or _handler_. When <FlamaName /> receives an HTTP request, its routing mechanism inspects the request's path and method to find a matching route. If a match is found, the associated handler function is called to process the request and generate a response.
...
- **API structure definition**: Routes define the accessible URLs of your application, forming the public interface or contract of your API.
- **Request dispatching**: They are crucial for directing incoming requests to the correct logic. Without routes, the application wouldn't know what to do with a request for `/users/` versus `/products/`.
- **Resource addressing**: In RESTful APIs, routes map to resources, allowing clients to interact with these resources using standard HTTP methods (e.g., GET to retrieve, POST to create).
- **Code organisation**: By associating specific functions with URL paths, routes help in organising the application's codebase into logical handlers for different functionalities.
...
<FlamaName /> offers flexible ways to define and organise routes, primarily through decorators on your handler functions
and by using a **Router** class for grouping related routes.
...
- Mounting this **product_router** to the main **app** at a specific path prefix (`/api/v1/shop`), so its routes become accessible at paths like `/api/v1/shop/products/` and `/api/v1/shop/product/{product_sku}/`.
