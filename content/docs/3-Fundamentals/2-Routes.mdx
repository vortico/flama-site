...
- **`@app.websocket_route(path, name=None)`**:
  ...
  ```python
  from flama import websockets # Typically at the top of your file
  ...
  Using the **`flama.websockets.WebSocket`** object as an asynchronous context manager (`async with websocket:`) is often cleaner as it handles **accept()** and **close()** automatically.
...
- **`app.add_websocket_route(path, endpoint, name=None)`**:
  ...
  async def my_websocket_handler(websocket: flama.websockets.WebSocket):
      async with websocket: # Handles accept and close
