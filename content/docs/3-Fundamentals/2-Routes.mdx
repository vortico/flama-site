...
For internal organisation, <FlamaName /> has the **`flama.routing.Router`** class. A **Router** allows grouping a set of related routes together.

- Routes are added to a **Router** by passing a list of `flama.routing.Route` instances to its `routes` parameter during initialisation, or by using its own `add_route()` method or route decorators (`@router.get()`, etc.) if used directly like an app instance for route definitions.
