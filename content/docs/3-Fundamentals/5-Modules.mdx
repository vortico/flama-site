...
Modules are a powerful and central concept in <FlamaName /> designed for organising and extending the framework's capabilities. They serve as dedicated units for managing related functionalities, configurations, and resources, particularly those with a lifecycle tied to the application itself. Modules are key to <FlamaName />'s extensibility, enabling the integration of external tools and services, such as databases like _SQLAlchemy_, or MLOps platforms like _MLflow_, effectively allowing you to create sophisticated plug-ins and deeply customise your application's behaviour.
...
2.  Via `app.modules`:
    All registered modules are also available in the **modules** collection (which behaves like a dictionary). You can access them using the module's name (if set).
...
# Define the Internationalisation Service
class I18nService:
...
# Define the Internationalisation Module
...
# Define sample translation data
sample_translations = {
    "en": {
...
# Define routes that will use the internationalisation service from the module
@app.route("/home")
async def home_page():
