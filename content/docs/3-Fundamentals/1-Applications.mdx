...
The **Flama** application instance is the heart of any web service or API built with the <FlamaName /> framework. It acts as the central orchestrator, bringing together all the different parts of your project—such as routes, components, modules, and middleware—into a single, coherent ASGI (Asynchronous Server Gateway Interface) application. Understanding its role and how to configure it is essential for developing with <FlamaName />.
...
1.  **Central orchestration**: It serves as the central point where all other parts of the <FlamaName /> framework (routes, components, modules, middleware) are connected and managed.
2.  **ASGI compliance**: It is the actual ASGI application that web servers interact with, handling the low-level details of the ASGI protocol.
3.  **Configuration hub**: It holds the application's global configuration, including settings for debugging, OpenAPI schema details, and paths for documentation.
4.  **Extensibility core**: It provides the mechanisms (like registration of modules and components) that make <FlamaName /> extensible and customisable.
5.  **Request lifecycle management**: It oversees the entire lifecycle of a request, from reception through middleware, routing, handler execution (with dependency injection), and response generation.
