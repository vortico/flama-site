...
2.  **A `__call__` method**:
    ...
    - This is the core of the middleware. It’s an asynchronous method that takes three arguments:
    ...
    Here's the basic structure of the `__call__` method:
...
Middleware is registered with a <FlamaName /> application instance by providing a list to its **middleware** parameter.
Each item in this list should be an instance of `flama.middleware.Middleware`.
The **Middleware** class is a helper that takes your custom middleware class and any arguments it needs for its `__init__` method.
...
# Create a Flama application and register your middleware
app = Flama(
    middleware=[
