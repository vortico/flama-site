...
- **family**: the artifact family, either `"ml"` for predictive models or `"llm"` for large language models. This is a
  required parameter.
...
compression format. Because `zstd` is provided directly by <FlamaName />'s Rust-powered core, it is available out of the
box on every supported <PythonName /> version, with no additional dependency to install.
...
flama.dump(model, path="model.flm", family="ml")

# Explicitly specifying the compression format
flama.dump(model, path="model.flm", family="ml", compression="lzma")  # Smallest file, slowest speed
flama.dump(model, path="model.flm", family="ml", compression="zlib")   # Good balance, no external dependency
flama.dump(model, path="model.flm", family="ml", compression="bz2")    # Standard library alternative
```
...
flama.dump(model, path="sklearn_model.flm", family="ml")
```
...
flama.dump(model, path="tensorflow_model.flm", family="ml")
```
...
flama.dump(model, path="pytorch_model.flm", family="ml")
```
