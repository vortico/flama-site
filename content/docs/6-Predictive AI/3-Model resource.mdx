...
Thus far we have shown how to [package models](/docs/predictive-ai/packaging-models/) in order to
[add them](/docs/predictive-ai/add-models/) to a <FlamaName/> App. The way in which ML models are added or what they
...
specifically a **MLResource** object, which represents an API resource in the REST sense of the word. This object
...
In what follows you will learn how to add model resources in a more customised way with the class **MLResource**.

## The MLResource class
...
- **MLResource**: This is a <FlamaName /> built-in class which derives from the most fundamental resource
  class **Resource**, and the [metaclass](https://docs.python.org/3/reference/datamodel.html#metaclasses) **MLResourceType**.
  ...
  Regarding **MLResourceType**, this is a <FlamaName /> built-in metaclass
  ...
  We agree with such a claim, which is why you won't need to worry about them because you won't even have to import **MLResourceType**,
  since this is already handled for you by the **MLResource** class.

- **ResourceRoute.method**: This is a <FlamaName /> built-in class method (of `ResourceRoute`) which allows us to
  convert a given class method into an **app** endpoint, with the following main arguments:
...
- **MySKModel(MLResource)**: The very first thing needed is to
  create our own model-resource class, which means inheriting from **MLResource**.
...
- **@ResourceRoute.method**: The first argument determines the path associated to the method which
...
schema type of the input and output arguments if we want any method to be <FlamaName /> compliant.
...
# Definition of input (X) and output (Y) schemas
...
- **pydantic.BaseModel**: We've added two schemas to define the data type of the input and the output using
  [Pydantic](https://docs.pydantic.dev/) models. <FlamaName /> uses Pydantic as its schema library, and
  schemas are declared as standard Pydantic models.

- **`t.Annotated[schemas.Schema, schemas.SchemaMetadata(...)]`**: This is the <FlamaName /> way of annotating
  handler parameters and return types with schema metadata. The `schemas.Schema` type tells the framework that
  the parameter should be validated against the schema, while `schemas.SchemaMetadata(X)` binds it to the
  concrete Pydantic model. This pattern replaces the older `app.schema.register_schema` approach.

- **predict(self, data: ...) -> ...**: Type hinting is mandatory, and (as we just mentioned) plays a key role in the
...
class MyModel(MLResource):
    ...
    def metadata(self):
        ...
        return {"metadata": {
            ...
            "custom": {**self.info, "date": str(datetime.now().date()), "time": str(datetime.now().time())},
        }}
```
...
```python
@app.models.model_resource("/model")
class MyModel(MLResource):
    ...
```
...
```python
@app.models.model_resource("/pytorch_model/")
class PyTorchModel(MLResource):
    name = "pytorch_model"
    ...
    @ResourceRoute.method("/metadata/", methods=["GET"], name="pytorch-metadata")
    def metadata(self):
        """
        tags:
            - PyTorch model
...
```python
@app.models.model_resource("/sklearn_model/")
class SKLearnModel(MLResource):
    name = "sklearn_model"
    ...
    @ResourceRoute.method("/metadata/", methods=["GET"], name="sklearn-metadata")
    def metadata(self):
        """
        tags:
            - ScikitLearn model
...
```python
@app.models.model_resource("/tensorflow_model/")
class TensorFlowModel(MLResource):
    ...
    @ResourceRoute.method("/metadata/", methods=["GET"], name="tensorflow-metadata")
    def metadata(self):
        """
        tags:
            - TensorFlow model
