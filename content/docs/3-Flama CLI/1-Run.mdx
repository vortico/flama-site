...
The path can be passed directly to the CLI as argument, or by utilising the equivalent environment variable
**FLAMA_APP**. Thus, if we are at **project_folder** (following the previous example of folder structure),
we can run the app by either of the following two ways:
