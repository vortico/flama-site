...
import FlamaName from 'src/components/FlamaName'
...
This is a meta-rule that runs all the utilities used for checking and applying <FlamaName /> coding standards, but it can be
done individually as follows:

### Code formatting

<FlamaName /> uses Black for formatting the code to adhere to the Black code
style ([PEP 8](https://peps.python.org/pep-0008/) compliant):
...
<FlamaName /> is completely static typed. To make sure your code fulfils this
constraint, you can check it using mypy:
...
Last, but not least, if you're adding new features to <FlamaName />, you need to include the tests required.
