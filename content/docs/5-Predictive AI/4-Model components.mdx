...
class MyCustomModel(BaseModel):
    ...
    def predict(self, x: typing.Any) -> typing.Any:
        ...
        # Ensure the output is JSON-serialisable (e.g. numpy int64 -> int)
        return [int(v) for v in result] if hasattr(result, "tolist") else result
```

As we can see, the **MyCustomModel** class inherits from the **BaseModel** class, which is a built-in class of <FlamaName />.
This base class provides the initialisation of the **model** attribute, and the **inspect** and **predict** methods,
which will be exposed as routes of our application. Note that the **predict** method includes a small conversion step
to ensure the output is JSON-serialisable, this is necessary because some ML frameworks (e.g. Scikit-Learn) return
numpy types which are not natively supported by JSON serialisation. If we wanted to instantiate a model object of the class
**MyCustomModel**, we would need to pass the model object as an argument to the constructor, as follows:

```python
model_artifact = flama.load(path="sklearn_model.flm")
