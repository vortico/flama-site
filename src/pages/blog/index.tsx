...
function BlogPreview({ blog }: BlogPreviewProps) {
  ...
  return (
    <article className="relative flex max-w-3xl flex-col space-y-4 lg:ml-auto xl:w-[50rem] xl:max-w-none">
      <div>
        <h3 className="text-xl font-bold tracking-tight text-zinc-700 dark:text-zinc-200">
          <a href={`/${blog.url}`}>{blog.title}</a>
        </h3>
        <div className="text-sm italic leading-7 dark:text-zinc-400 lg:absolute lg:top-0 lg:right-full lg:mr-8 lg:whitespace-nowrap">
      ...
      <div>
        <a
          className="dark:highlight-white/5 inline-flex h-7 items-center rounded-full bg-white px-4 shadow-sm ring-1 ring-brand-900/10 hover:ring-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500 dark:bg-zinc-700 dark:ring-0 dark:hover:bg-zinc-600"
          href={`/${blog.url}`}
        >
