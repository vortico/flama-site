...
export default function Window({ title, contentRef, className, children }: WindowProps) {
  ...
  return (
    <div
      className={state === 'full' ? 'fixed inset-0 z-[200] h-screen w-screen' : 'relative h-full w-full'}
    ...
    >
      ...
      >
        ...
        <div ref={contentRef} className="mt-px h-[calc(100%-31px)] w-full overflow-auto">
          {children}
        </div>
      </div>
    </div>
